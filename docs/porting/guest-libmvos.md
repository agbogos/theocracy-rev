...
- **Flag/EnvSystem/singleton sync** (game↔mvos COPY split — a workaround; the
  permanent linker fix landed in [G12](#g12--r_386_copy-shared-storage-done) and
  deleted all of it)
...
buttons (`+0xa8`) and pointer-sprite state, which makes it a **likely cause of
the cursor ghost-trail artifacts** reported on post-game screens (Credits, Load
Game) — unconfirmed, pending retest.
...
Verified by measuring captured frames, not by eye alone: the 4:3 movie has

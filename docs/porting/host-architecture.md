...
`port/CMakeLists.txt` builds one executable, `theoc`, from `main.cpp machine.cpp guestlink.cpp traps.cpp video.cpp mpeg.cpp blit.cpp`, linking `unicorn SDL2 avformat avcodec avutil swscale swresample` from `/opt/homebrew`. C++17, `-Wall -Wextra -Wno-unused-parameter`. There is no test target and no library target.

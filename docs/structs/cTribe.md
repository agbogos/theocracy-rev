...
A faction. `new(0x84)` (132 bytes), one per slot in
...
| `0x34` | `i32 [5]` | `resource` | Five per-tribe resource/mana pools. Init `g_TribeResourceInit` (`0x84c85f1`); `cTribe_AddResource(amt, idx)` adds & clamps to `g_TribeResourceMax` (`0x84c85f5`). Both globals are scalars rather than per-resource tables, so all five pools share one starting value and one cap. |
| `0x48` | `ptr` | `controller` | Per-tribe AI and order state; allocated elsewhere, freed+nulled by `cTribe_FreeController`. Sub-objects at `controller+0x2c/+0x30/+0x50`. |
| `0x4c` | `ptr` | `template` | Static tribe definition = `DAT_08645240 + id*0x10`. Decoded as 4× `int32` — see the roster template below. |
...
- 0–5 — the playable realms (start `unknown` to each other; must meet; full
  diplomacy).
- 6, 7, 8 — minor factions (6 starts at relation `1`; 7/8 default `3`).
- 9, 10 — always allied (`5`) to everyone, so neutral or independent rather than
  competing.
...
Filled once by `InitTribeRoster` (`0x817bdd0`) via `TribeTemplate_Set4(entry, a,
b, c, d)`. Each of the 11 entries (`DAT_08645240 + id*0x10`) is four `int32`s:
...
The four ints read as per-tribe difficulty parameters on a percentage scale,
with the neutrals at a flat 100% baseline — an inference from the shape of the
values, not from a reader. Pinning down what each of the four means needs either
the raw `.data` values (visible in Ghidra at `0x84c817c`) or a `tribe->template`
reader. `InitTribeRoster` is general game-data init rather than tribe-only: it
also sets up the `flyinghelp1/2` tooltip locale entries and the `small_1pix`
font.

## Still open

- `+0x01..+0x07`, `flag0`/`_flags`, and the `+0x6e..+0x83` tail (~22 bytes) —
  likely AI bookkeeping. Trace the resource getters and the users of
  `controller` (`+0x48`) next.
- What each of the roster template's four `int32`s means. The layout is decoded
  above; the semantics are not.
- Where the human-readable faction identities live. They are not in the roster
  template, which holds only the four numbers.

...
engine shared library. Running document — kept concise; append findings as we
go.

(Elsewhere these docs call the binaries "1999" — that is their build vintage,
not the game's release year. Both are used deliberately.)
...
- **Toolchain:** GCC 2.x (g++ 2.95-era, GNU v2 mangling, sjlj exceptions).
  ...
  (huge win). The game binary imports 232 of them.
...
Game **data is not in this folder** — installed from CD by `inst.linux` into
`/usr/games/theocracy_base/data`. The CD has since been provided (`data/cd/`)
and the packs extracted to `data/game/` — see
[reference/phls-format.md](reference/phls-format.md); no longer a blocker.
...
15/16bpp first-class) — no planar heritage — so this is Amiga-fluent devs
...
**The "Amiga-fluent devs" half of that was a surmise from the idioms, and the
studio history supplies the mechanism** ([history.md](history.md)): in **1997
Philos hired several developers off *Perihelion: The Prophecy*, a 1993 Amiga
RPG**, and that team became Theocracy's core. So the executive model here is the
habit of people who had just spent years writing for AmigaOS. Note this
*strengthens* the conclusion already drawn rather than changing it — it is still
not a converted Amiga codebase, and the chunky-LFB pixel pipeline is still the
evidence for that.
...
## Progress
- **Memory & containers — first pass done.** See
...
- **Application bootstrap — first pass done (since corrected).** See
  ...
  **SMPEG**, device backends **dlopen'd** (shipped family is **X11**, see
  ...
  348 symbols back).
- **Audio / threads / processes — first pass done.** See
  ...
  fork/execlp.
- **Video plugin — fully decompiled.** See
...
- **Port — PLAYABLE (guest-libmvos):** both `theocracy.real` and real
  `libmvos.so` run under Unicorn; only the OS/library boundary is HLE'd.
  Single-player is playable end-to-end. See
...
split three ways: the `cApplication` bootstrap is **done** (see above), the
`cGD` backend dispatch went to

...
(Ghidra base `0x10000`). All of this sits *behind* the HLE boundary, so it is
documented here as the behavioural contract a host has to satisfy.
...
4. Two buffers, both via `__builtin_vec_new`:
   - `+0x4c` — ring/back buffer, `rate/10` samples (100 ms) × channels ×
     sample size (`Sample_Size[fmt]`, 1 or 2; anything else →
     `Fatal("Illegal format")`). Sample count is cached at `+0x50`, rate at
     `+0x40`.
   - `+0x34` — the software mix accumulation buffer, `rate/10` × channels × 4,
     giving 32-bit headroom per sample. Any previous one is `__builtin_delete`d
     first. Alongside it: `+0x39` = `Sample_Size[fmt]`, `+0x3a` = channels,
     `+0x38` = 0.
5. Inherits `cSoundCard_SoftwareMix` (all mixing in software, with a
   `cSoundChannel_SoftwareMix` per voice) and `cThread`. The ctor ends with
   `cThread::Launch(this + 4)`, so the mixer runs on its own thread, writing
   mixed blocks to the fd.

> **ABI correction.** This doc previously said the ctor ends
...
**How the port runs this.** The pure-HLE plan was to replace the mixer with
native code behind a pull-callback. That is not what happened. Under
[guest-libmvos](../porting/guest-libmvos.md) the engine's own mixer runs as
guest code, with `/dev/dsp` HLE'd onto SDL audio.

The host is single-threaded, because a nested `uc_emu_start` crashes Unicorn, so
the mixer cannot have a host thread. It is green-run instead:
`cSoundCard_Linux::Main` is patched to mix one-shot, and a slice is spliced into
the running emulation through `redirect_guest` whenever the audio queue drains
below its target.

That target is a tuning knob, not an implementation detail. See
[frame-timing.md](../porting/frame-timing.md), Bug 3 — the mixer was originally
serviced only at present, which coupled audio to the frame rate. The format and
rate negotiation above still happens for real, in the guest's own code.

## Threads — `cThread` (ctor @ `0xa5860`, `Launch` @ `0xa58c0`)

- Object: `{+0x00 ?, +0x04 cStream vtable base, +0x10 running flag, +0x14
  pthread_t}`. `cThread` derives from `cStream`, the message-pipe endpoint.
- `Launch`: `cPipe::CreatePipe()` (comms channel, fds at `+0x08/+0x0c`) then
  ...
  run method, so game subclasses run guest code — a green thread under the
  emulator.
...
`setitimer`/SIGALRM drives `cTimerSystem_Linux`, and consumers register as
`cVTimer` subclasses. The `cIntuition` ctor (`0x9d370`) registers itself with
the global `TimerSystem` through vcall `+0x18/+8`, and `Fatal`s with "Unable to
activate timer" if that fails.

Input polling and repaint are therefore timer-driven.
`NoTimerInterruptPaintFlag` gates repaint during mode switches — see
...
mis-flagged noreturn, so the decompiler truncates functions at the first call
with "WARNING: Subroutine does not return". `Fatal` genuinely is noreturn; the
others are not, so fix their signatures in the Ghidra DB. Until that is done,
read such decompiles as "continues after the call". Same class of artifact as
the one in [memory-and-containers.md](memory-and-containers.md).

...
Where the concrete application lives. Addresses in the **game executable** (load
...
2. **Single-instance lock:** open IPC on `localhost:5043` (port `0x13b3`); if
   taken → `Fatal("...You can run only one Theocracy in the same time!")` (note
   the Hunglish — Hungarian devs).
...
- **`MainMenu_Run`** (`0x812e980`): builds & runs the main menu. Sets 800×600,
  ...
  localized text (`cLocaleEntry`) and a click `cSample`. Builds the

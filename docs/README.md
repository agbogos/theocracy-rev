...
| **Windows port** | **DONE 2026-08-04 — playable, confirmed by play.** Three hosts now run the same i386 binaries from the same source, with no `#ifdef` outside `traps.cpp`. Getting there took exactly two things beyond the build. (1) The single-instance lock fake returned success from `bind` **without binding**, which POSIX forgave because `listen()` auto-binds an unbound socket and Winsock does not — **a latent bug in the macOS and Linux ports too**, an implicit `bind` nobody had chosen to depend on. Fixed by binding an ephemeral loopback port; the ignored-`abort` runaway it exposed is now bounded (`THEOC_ABORT_CAP`). (2) **Sub-millisecond sleep**, the risk ranked first and the only one that was real: the 15.6 ms floor cost a 94 ms province frame against an 83.3 ms target (~13% slow, and *game speed* rather than jitter, because the limiter is elapsed-based). `theoc_sleep_us()` on `CreateWaitableTimerEx(HIGH_RESOLUTION)` fixed it — measured in-game at **87.0 ms (+4.4%)**, with the residual decomposed to ~1.0 ms per slice, i.e. the timer's own floor rather than a defect; no `timeBeginPeriod` needed. A 6-minute instrumented run (realm 60 fps against the 16 ms cap — unreachable under `Sleep`'s granularity, and the cleanest proof the timer works; 0 unimplemented traps over 3.58M import calls; flat heap) plus save/load and a full netgame played. Path handling, the standing prime suspect, never bit. Residual unknowns are documented, not open: timing under contention and on bare metal is still one idle VM's worth, and province audio is marginally noisier than macOS (≤17 ms/s, inaudible). **2026-08-04, later that day — the timing residual is answered and a regression was found.** The two contention runs came back: the **sleep-primitive question is closed**, the waitable timer holds to 2× oversubscription (ticks/frame 2.87–2.93, never collapsing; still fires a 0.1 ms sleep in 0.84 ms where `Sleep()+timeBeginPeriod` needs 11 ms), and no code change is indicated. What contention costs is *CPU availability*, not resolution: with no idle core the province frame stretches 83 → ~98 ms (12 fps → ~10.2). One pre-declared criterion was breached and turned out to be **mis-specified** — it assumed per-slice overshoots accumulate, and the loop is self-limiting instead (4× worse single sleeps moved the frame 0.9 ms). Only bare metal is still outstanding. Separately, the same runs surfaced that **`WSAStartup` was never called**: Winsock only worked because the full-fat ffmpeg's DLLs initialised it, so the [bundle-size work](porting/other-os-ports.md#both-bundles-minus-the-ffmpeg-nobody-uses) silently broke every socket — the identical `listen()`-auto-binds mistake, one layer out. Fixed, and `socket()` now logs its failures instead of only its successes, which is what made a host-side error read as "You can run only one Theocracy in the same time!". **Re-verified by play on the fixed build the same day: boots, multiplayer works, cutscenes play** — the last also being the first end-to-end confirmation of the minimal ffmpeg on Windows. Nothing in the Windows port now rests on a pre-fix result. Only the bare-metal timing run is outstanding (~2026-08-18). |
| **Windows port — build** | **BUILDS 2026-08-03.** `tools/package-windows.sh` cross-builds from macOS with mingw-w64 and produces `dist/theoc-windows-x64/` (**7.3 MB** since the 2026-08-04 minimal-ffmpeg work, from 119 MB; seven DLLs from the real import closure, timing probe included); no Windows machine is involved in making it. `traps.cpp` needed 11 fixes, of which the load-bearing ones were: Winsock SOCKETs are a **separate namespace from CRT fds** (so `read`/`write`/`close` had to learn which is which — libmvos polls sockets through plain `read`/`write`), `O_BINARY` everywhere (CRLF translation would silently corrupt saves and packs), and a `WSAGetLastError`→Linux-errno table. Cross-compiling also found a **real latent bug in the working macOS build** — `video.cpp` used `std::floor` without `<cmath>`. It runs the *Linux* binary. See [porting/other-os-ports.md](porting/other-os-ports.md) |

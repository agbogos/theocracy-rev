...
| **macOS port — next** | The three host cleanups and the long-session harness (`THEOC_LONGRUN`) are done; what remains under playability is the multi-hour session itself, which is a human test. Then modernisation (decouple sim from render). Full list: `../task_fifo.md` |

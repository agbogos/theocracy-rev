...
| **Windows port** | **scaffolded 2026-08-03, no host machine involved yet** — mingw-w64 cross toolchain, plus measurements that moved three of the five audited risks: `main.cpp` also cross-compiles clean, and *every* POSIX header mingw lacks is a socket header, so filesystem and clock largely dissolve while sockets are confirmed as the only structural work. The exception is timing: `usleep` **links**, which is the trap rather than the reprieve, since it says nothing about the sub-millisecond granularity the sliced sleep model needs. `tools/win_timing_probe.cpp` measures that standalone — written, cross-compiles, **not yet run**. It runs the *Linux* binary. See [porting/other-os-ports.md](porting/other-os-ports.md) |

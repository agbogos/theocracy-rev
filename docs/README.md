...
- [porting/heap-growth-trials.md](porting/heap-growth-trials.md) — **the leak hunt, by elimination.** Why a fifth mixed-activity long session would answer nothing, the one-activity-per-run protocol built on `Alt+M` markers, and four trials that rule out idling, the window mode and the realm↔province sawtooth (a warm-up that saturates at ~8 KB/cycle, not the leak it looked like). Reference costs for reading any future log, and the eight instrument defects the trials exposed — five of which produced plausible wrong numbers rather than obvious failures.
...
| **macOS port — next** | The host cleanups and the long-session harness are done, and the harness has grown `Alt+M` markers plus a guest-work column. What remains under playability is the **heap-leak hunt**: four controlled trials have eliminated idling, the window mode and the screen cycle without finding the +7–11 MB/h of sustained play — battles are next ([porting/heap-growth-trials.md](porting/heap-growth-trials.md)). Then modernisation (decouple sim from render). Full list: `../task_fifo.md` |

...
- [porting/other-os-ports.md](porting/other-os-ports.md) — **Windows and Linux hosts**, structured from an audit of `port/src` rather than porting lore. The reusable core (four of seven units already travel; all of `docs/` transfers); three things that look like blockers and are not (`fork`/`execlp` are stubs, one host thread, no host `mmap`); why **Linux is a subtraction** — the BSD translations are written host-macro → guest-constant, so on Linux they are the identity and there is nothing to neutralise (measured on both platforms, correcting an earlier inference that they would be *actively wrong*) — and the one thing that must stay; Windows' real risk order, led by sub-millisecond sleep against a ~15.6 ms scheduler; and **which binary Windows should run** — the Linux one, since the Linux binaries ship unprotected while the Windows build does not run as shipped on modern Windows, and a Windows executable would be a different compilation to which none of this repo's addresses apply.
...
| **Windows port** | **BOOTS 2026-08-03 — reaches `Start`, then hit the single-instance lock.** First run was line-for-line identical to macOS through the entire boot (79 COPY relocs, 215/215 ctors, all nine subsystem flags, plugins, audio, `cIntuition`) and then looped: the lock fake returned success from `bind` **without binding**, which POSIX forgave because `listen()` auto-binds an unbound socket and Winsock does not. **That was a latent bug in the macOS and Linux ports too** — an implicit `bind` nobody had chosen to depend on. Fixed by binding an ephemeral loopback port; the ignored-`abort` runaway it exposed is now bounded (`THEOC_ABORT_CAP`). The timing probe **ran**: the 15.6 ms floor is real (province frame 94 ms vs 83.3 target), `CreateWaitableTimerEx(HIGH_RESOLUTION)` fixes it (84.0 ms, 0.65 ms heartbeat lateness) and `timeBeginPeriod` is then unnecessary — measured in a VM, and **not yet implemented in the port**. Retest pending. |
| **Windows port — build** | **BUILDS 2026-08-03.** `tools/package-windows.sh` cross-builds from macOS with mingw-w64 and produces `dist/theoc-windows-x64/` (119 MB, seven DLLs from the real import closure, timing probe included); no Windows machine is involved in making it. `traps.cpp` needed 11 fixes, of which the load-bearing ones were: Winsock SOCKETs are a **separate namespace from CRT fds** (so `read`/`write`/`close` had to learn which is which — libmvos polls sockets through plain `read`/`write`), `O_BINARY` everywhere (CRLF translation would silently corrupt saves and packs), and a `WSAGetLastError`→Linux-errno table. Cross-compiling also found a **real latent bug in the working macOS build** — `video.cpp` used `std::floor` without `<cmath>`. Sub-millisecond sleep, the top risk, remains **unmeasured**: `tools/win_timing_probe.cpp` is written and cross-compiles but needs a Windows host. It runs the *Linux* binary. See [porting/other-os-ports.md](porting/other-os-ports.md) |

...
| **Windows port** | **RUNS 2026-08-04 — past `Start`, into the game.** Three hosts now run the same i386 binaries from the same source. Getting there took exactly two things beyond the build. (1) The single-instance lock fake returned success from `bind` **without binding**, which POSIX forgave because `listen()` auto-binds an unbound socket and Winsock does not — **a latent bug in the macOS and Linux ports too**, an implicit `bind` nobody had chosen to depend on. Fixed by binding an ephemeral loopback port; the ignored-`abort` runaway it exposed is now bounded (`THEOC_ABORT_CAP`). (2) **Sub-millisecond sleep**, the risk ranked first and the only one that was real: the 15.6 ms floor cost a 94 ms province frame against an 83.3 ms target (~13% slow, and *game speed* rather than jitter, because the limiter is elapsed-based). `theoc_sleep_us()` on `CreateWaitableTimerEx(HIGH_RESOLUTION)` shipped 2026-08-04 — 84.0 ms frame, 0.65 ms heartbeat lateness, no `timeBeginPeriod` needed (measured: it adds nothing). Path handling, the standing prime suspect, never bit. Still open: **a proper playtest** and screenshot comparison, multiplayer, and timing under load (`--busy N` and bare metal are both unmeasured). |
